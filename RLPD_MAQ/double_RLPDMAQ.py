...
from VQVAE.modules import VectorQuantizedVAE, RawVectorQuantizedVAE
...
class MAQDBDRLPDSACAgent(RLPDDiscreteSACAgent):
	def __init__(self, config):
		super(MAQDBDRLPDSACAgent, self).__init__(config)
		self.config = config
		...
		self.general_vqvae_model = RawVectorQuantizedVAE( seq_len=vqvae_config['sequence_length'], K=vqvae_config['double_k'], dim=vqvae_config['hidden_size'], output_dim=actions.shape[1]).to(self.device)	
		
		
		# loading the last 
		ls = [filename for filename in os.listdir(config['vqvae_model_path']) if filename.endswith(".pth") and filename.startswith("model") and not "second" in filename ]
		...
		# load second
		ls = [filename for filename in os.listdir(config['vqvae_model_path']) if filename.endswith(".pth") and filename.startswith("model") and "second" in filename ]
		...
		print("loaded second vqvae model",last_vqvae_cp)
		
		load(self.general_vqvae_model, os.path.join(config['vqvae_model_path'], last_vqvae_cp ))
		self.general_vqvae_model.eval()
		print("second VQVAE model loaded.")
		...
		self.conditioned_k = vqvae_config['k']
		self.N_action = vqvae_config['k']+vqvae_config['double_k']
		print("num macro action: ", self.N_action, "k: ", vqvae_config['k'], "double_k: ", vqvae_config['double_k'])
	...
	def decoded_primitive_actions(self, states, K_index):
		...
		with torch.no_grad():
			# if K_index > conditioned_k then use self.general_vqvae_model else use self.vqvae_model, K_index might be tensor with different dimension
			# k_index is like  tensor([37, 12, 72,  9, 75,  5, 79, 64], device='cuda:0')
			decoded_actions = []
			for i in range(len(K_index)):
				# make a tensor of K_index[i]
				if K_index[i] >= self.conditioned_k:
					index = torch.tensor([K_index[i] - self.conditioned_k], device=self.device)
					# print(index)
					decoded_actions.append(self.general_vqvae_model.forward_decoder(index)[0])
				else:	
					index = torch.tensor([K_index[i]], device=self.device)
					# state is like tensor([21], device='cuda:0') convert to tensor([1, 21])
					state = torch.tensor(states[i], device=self.device)
					state = state.unsqueeze(0)
					# print(index)
					decoded_actions.append(self.vqvae_model.forward_decoder(state, index)[0])
			
			# turn into tensor
			#     decoded_actions = torch.stack(decoded_actions)
			# TypeError: expected Tensor as element 0 in argument 0, but got numpy.ndarray
			# print(decoded_actions) # decoded_actions is list of ndarray turn into tensor
			decoded_actions = torch.tensor(decoded_actions, dtype=torch.float32, device=self.device)
	...
	def train(self):
		...
		while self.total_time_step <= self.training_steps:
			...
			if self.total_time_step % self.eval_interval == 0 and self.total_time_step > 0:
				avg_score, norm_avg_score, avg_general_vqvae_usage, avg_steps, avg_success = self.evaluate()
				...
				self.writer.add_scalar('Evaluate/General_VQVAE_Usage', avg_general_vqvae_usage, self.total_time_step)
				self.writer.add_scalar('Evaluate/Steps', avg_steps, self.total_time_step)
				self.writer.add_scalar('Evaluate/Successes', avg_success, self.total_time_step)
				
				# Write data to csv file (adjust fields)
				csv_path = os.path.join(self.writer.log_dir, 'experiment_data.csv')
				file_exists = os.path.isfile(csv_path)
				with open(csv_path, 'a', newline='') as f:
					fieldnames = ['training_step', 'episode_reward', 'norm_episode_reward', 'general_vqvae_usage', 'success'] # Removed BC fields
					writer = csv.DictWriter(f, fieldnames=fieldnames)
					if not file_exists:
						writer.writeheader()
					writer.writerow({'training_step': self.total_time_step, 'episode_reward': avg_score, 'norm_episode_reward': norm_avg_score, 'general_vqvae_usage': avg_general_vqvae_usage, 'success' : avg_success})
	...
	def evaluate(self):
		...
		success = [False] * self.eval_episode
		normalized_reward = [0] * self.eval_episode
		genearl_vqvae_usage = [0] * self.eval_episode
		steps = [0] * self.eval_episode
		...
		while True:
			...
			for i in range(self.eval_episode):
				if actions[i] >= self.conditioned_k:
					genearl_vqvae_usage[i] += 1
				steps[i] += 1
				if terminates[i] and not all_done[i]:
					
					if infos[i].get('goal_achieved', False) or infos[i].get('success',False):
						success[i] = True

					normalized_reward[i] = self.dummy_env.get_normalized_score(episode_rewards[i]) * 100
					print(f"env {i} terminated, reward: {episode_rewards[i]} | normalized reward: {normalized_reward[i]} | general usage: {genearl_vqvae_usage[i]} ({genearl_vqvae_usage[i]/steps[i]*100}%) | steps: {steps[i]} | Success: { 'success' if success[i] else 'failed'}")
		...
		normalized_avg = np.mean(normalized_reward)
		print(f"average score: {avg}  average normalized score: {normalized_avg}  average success: {np.mean(success)}")
		print("==============================================")
		return avg, normalized_avg, np.mean(genearl_vqvae_usage), np.mean(steps), np.mean(success)
