...
usage() {
    ...
    echo "  -dkbsz, --double_codebook_size      VQVAE double codebook size"
...
MAQ_VQVAE_TRAINING() {
    ...
    local double_codebook_size="$5"
    local training_source="$6"
    local testing_source="$7"
    local suffix="$8"
    local seed="$9"
    ...
    echo "Double Codebook Size: $double_codebook_size"
    ...
    CUDA_VISIBLE_DEVICES="$gpuid" python3 double_vqvae_train.py  --env "$environment_name" \
                        --suffix "$suffix" \
                        --seed "$seed" \
                        --seqlen "$sequence_length" \
                        --k "$codebook_size" \
                        --double_k "$double_codebook_size" \
...
MAQ_RL_TRAINING() {
    ...
    if [ "$method" == "MAQ+RLPD" ]; then
        ...
        cd RLPD_MAQ || exit
        CUDA_VISIBLE_DEVICES="$gpuid" python3 double_main.py --env "$environment_name" \
                        --suffix "$suffix" \
                        --seed "$seed" \
                        --seqlen "$sequence_length" \
                        --training_dataset "$training_source" \
...
while :; do
    case $1 in
        ...
        -dkbsz|--double_codebook_size) shift; double_codebook_size="${1}" ;;
...
if [ "$training_method" == "MAQ+DSAC" ] || [ "$training_method" == "MAQ+RLPD" ] || [ "$training_method" == "MAQ+IQL" ]; then
    ...
    suffix="seed$seed"_"sq$sequence_length"_"k$codebook_size"_"dk$double_codebook_size"
    if [ "$tag" != "" ]; then
        suffix="${suffix}_$tag"
    fi
    ...
    MAQ_VQVAE_TRAINING "$training_method" "$environement_name" "$sequence_length" "$codebook_size" "$double_codebook_size" "$training_source" "$testing_source" "$suffix" "$seed"
    MAQ_RL_TRAINING "$training_method" "$environement_name" "$sequence_length" "$codebook_size" "$double_codebook_size" "$training_source" "$testing_source" "$suffix" "$seed"
