...
For example, the following command demonstrate training MAQ+RLPD with macro action length=9 and codebook size=16 on door task:
```
./scripts/train.sh --method "MAQ+RLPD" --sequence_length 9 --codebook_size 16 --environment "door-human-v1" --seed 1
```
...
```
You will obtain a another folder in the same parent path:
```
DSAC_MAQ
└── log/
    └── exp_seed1_sq9_k16/
...
#### MAQ+IQL and IQL
We build the IQL and MAQ+IQL based on the [IQL-PyTorch](https://github.com/gwthomas/IQL-PyTorch).
...
#### SAC
We build the SAC based on the [stable-baselines3](https://github.com/DLR-RM/stable-baselines3).
For example, the following command demonstrating trainig SAC on door task with seed 1:
```
./scripts/train.sh --method "SAC" --environment "door-human-v1" --seed 1
```
...
#### RLPD
We build the RLPD based on [ikostrikov's repository `RLPD`](https://github.com/ikostrikov/rlpd).
We also make small changes on the dataset loading, making it compatible with the customized datasets.
Before training RLPD, you need to build the environment for training RLPD, you can reference their instructions or run the following command on our container:
```bash
# in workspace/
...
pip install git+https://github.com/ikostrikov/dmcgym.git
```
For training RLPD with your datasets, you can use following commands:
```bash
...
We also make a simple command to run the experiment with a specific macro action length and codebook size, and method. You can use the following command:
```bash
./scripts/lazy_rerun.sh
```

> [!NOTE]
> This command will run all the combintation of macro action length and codebook size and methods to reproduce the experiment in section 5.2.3, it may take several hours to finish all the combinations (108 experiments in total).
...
After training methods, you can evaluate the methods by using the following command:
```bash
# For MAQ based methods
# Section 5.2.2: MAQ+RLPD in door task (with macro action length=9 and codebook size=16)
...
> [!NOTE]
> For evaluating RLPD, you need to specifiy the model path of the RLPD model to be evaluated, other baseline methods are able to be trained with `./scripts/train.sh` , so they are able to automatically locate the model path by itself.
