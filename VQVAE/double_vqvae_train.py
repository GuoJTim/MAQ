...
from modules import VectorQuantizedVAE, to_scalar, RawVectorQuantizedVAE
...
def train(config):
    ...
    model.double()
    model_second = RawVectorQuantizedVAE(seq_len=config["sequence_length"], K=config['double_k'], dim=config['hidden_size'], output_dim=actions.shape[1]).to(device)
    model_second.double()

    optimizer = torch.optim.Adam(model.parameters(), lr=config['lr'])
    optimizer_second = torch.optim.Adam(model_second.parameters(), lr=config['lr_second'])
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda epoch: 1/(epoch+1))
    scheduler_second = torch.optim.lr_scheduler.LambdaLR(optimizer_second, lambda epoch: 1/(epoch+1))
    writer.add_scalar('lr', scheduler.get_last_lr()[0], 0)
    writer.add_scalar('lr_second', scheduler_second.get_last_lr()[0], 0)
    ...
    train_indices_counter_second = 0
    test_indices_counter = 0
    test_indices_counter_second = 0
    criterion = torch.nn.MSELoss()
    for epoch in range(config['epoch']):
        model.train()
        model_second.train()
        ls = []
        for i, (state, dataset_action) in enumerate(train_dataloader):
            # print(images[0])
            images = state.to(device, dtype=torch.float64)   # (B, state_dim)
            action = dataset_action.to(device, dtype=torch.float64)  # (B, sequence_len, action_dim)
            ...
            action = action.view(action.size(0), -1)
            x_tilde, z_e_x, z_q_x, indices = model_second(action)

            # Reconstruction loss
            loss_recons_second = criterion(x_tilde, action)

            # Vector quantization objective
            loss_vq_second = F.mse_loss(z_q_x, z_e_x.detach())
            # Commitment objective
            loss_commit_second = F.mse_loss(z_e_x, z_q_x.detach())
            
            loss_second = loss_recons_second + loss_vq_second + config['beta_second'] * loss_commit_second
            # ls.append(str(loss.item()))
            loss_second.backward()

            # Logs
            writer.add_scalar('loss/train_second/reconstruction', loss_recons_second.item(), steps)
            writer.add_scalar('loss/train_second/quantization', loss_vq_second.item(), steps)
            writer.add_scalar('index/train_second', indices.cpu().detach().numpy()[0], train_indices_counter_second)

            optimizer_second.step()
            steps += 1
            train_indices_counter_second += 1
        ...
        if (epoch+1) % 5 == 0:
            ...
            for i, (state, action) in enumerate(test_dataloader):
                ...
                # print("predict",actions.shape[1],x_tilde.shape,x_tilde[0])
                total_loss_second += eval_mse_loss.item()

                writer.add_scalar('index/test_second', indices.cpu().detach().numpy()[0], test_indices_counter_second)
                test_indices_counter_second += 1
                
            print('=========================================')
            print(f'Eval MSE Loss: {total_loss / total_count}')
            print(f'Eval MSE Loss second: {total_loss_second / total_count}')
            save(model, os.path.join(writer.log_dir, f"model_{epoch+1}_{total_loss/total_count}.pth"))
            save(model_second, os.path.join(writer.log_dir, f"model_second_{epoch+1}_{total_loss_second/total_count}.pth"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--k', type=int, default=16,
                        help='Codebook size')
    parser.add_argument('--double_k', type=int, default=32,
                        help='Codebook size')
